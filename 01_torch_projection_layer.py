...
class Linear3DOperatorForwardLayer(torch.autograd.Function):
    """
...
class Linear3DOperatorAdjointLayer(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
...
fwd_layer = Linear3DOperatorForwardLayer.apply
adjoint_layer = Linear3DOperatorAdjointLayer.apply

f1 = fwd_layer(x, projector)
print('forward projection (Ax) .:', f1.shape, type(f1), device(f1))


b1 = adjoint_layer(y, projector)
print('back projection (A^T y) .:', b1.shape, type(b1), device(b1))

fb1 = adjoint_layer(fwd_layer(x, projector), projector)
